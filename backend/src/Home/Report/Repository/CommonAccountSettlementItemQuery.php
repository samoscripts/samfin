...
class CommonAccountSettlementItemQuery
{
    ...
    public function fetchItemsForPeriod(
        ...
        $sql = <<<SQL
            ...
            WHERE t.operation_date >= :dateFrom
              ...
              AND {$statusFilter}
              AND (
                  t.paid_from_party_id = :commonPartyId
                  OR t.paid_to_party_id = :commonPartyId
              )
        ...
        $rows = $this->em->getConnection()->fetchAllAssociative($sql, [
            'dateFrom'       => $dateFrom,
            'dateTo'         => $dateTo,
            'commonPartyId'  => $commonAccountPartyId,
        ]);
