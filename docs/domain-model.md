...
|---------|-------------------|
| `trans_date` | Data transakcji (rzeczywista; przy kartach mBank może różnić się od daty księgowania) |
| `booking_date` | Data księgowania z wyciągu (format elektroniczny mBank) |
| `trans_title` | Tytuł transakcji z banku (#Tytuł w formacie elektronicznym, po oczyszczeniu) |
| `trans_description` | Opis transakcji z banku (kolumna „Opis operacji” w eksporcie elektronicznym mBank); niemodyfikowalny po utworzeniu |
| `trans_custom_description` | Opcjonalny własny opis użytkownika; edytowalny przy tworzeniu i edycji; wyszukiwalny przez filtr `description` |
| `balance_after_minor` | Saldo po operacji w groszach (format elektroniczny mBank) |
| `counterparty_name` | Nazwa kontrahenta z wyciągu |
...
**MVP (ADR-019):**
- Wymagane: `direction`, `trans_date`, `amount_minor`, `trans_description`
...
**Warunek `amount_minor`:** w UI wpisuje się kwotę w PLN (wartość bezwzględna, np. `800` lub `21,56`); w `conditions_json` zapisywane są dodatnie grosze (`80000`, `2156`). Matcher porównuje `abs(kwota transakcji)` z wartością warunku (kierunek jest już w pierwszym warunku `direction`). Lista wartości (`in` / `not_in`): separator `;` (np. `21,56; 45,15`).
...
## Usuwanie transakcji (ADR-030)

`DELETE /api/transactions/{id}` — przed skasowaniem wiersza z `transactions` pełny snapshot trafia do `transactions_trash`. Usunięta transakcja znika ze wszystkich zapytań operacyjnych (lista, stats, reguły, rozliczenia po `markDirty`). UI: przycisk „Usuń” w panelu szczegółów z `ConfirmDialog`.
