...
### Struktura katalogów = struktura menu

Katalogi FE i BE powinny odzwierciedlać **faktyczny podział funkcjonalny** widoczny w menu (ADR-040), nie tylko wygodę importów:

| Moduł | Wzorzec FE | Wzorzec BE |
|-------|------------|------------|
| Raporty | `reports/analytics/`, `reports/breakdown/`, … | `Report/Analytics/`, `Report/Breakdown/`, … |
| Konfiguracja | `configuration/general/`, `configuration/rules/`, `configuration/dashboard/` | `Configuration/General/`, `Configuration/Rules/` |

- Foldery domenowe — **angielskie** (`general`, `rules`); segmenty URL — **polskie** (`/konfiguracja/ogolne/podmioty`).
- Nie umieszczaj funkcji z jednego podmenu w katalogu innego (np. raportów w `configuration/general/`).
- Przy nowym podmenu: layout + podkatalog + wpis w sidebarze (`NavGroup`) + aktualizacja `docs/modules.md`.
...
`Layout` renderuje `<Outlet />` bez paddingu. Strony **bezpośrednio** pod `Layout` (np. `TransactionEdit`, `Dashboard`) owijają content w `p-4 md:p-6 max-w-screen-xl`. Strony pod layoutem modułu (`ConfigurationLayout`, `GeneralLayout`, `ImportLayout`) dostają padding od rodzica — nie duplikować. Szczegóły: `.cursor/rules/frontend-page-padding.mdc`.
...
**CI:** przy push/PR na `main` — workflow [`.github/workflows/tests.yml`](../.github/workflows/tests.yml) (PHPUnit + Vitest + migracje na MariaDB 11).
...
Uruchomienie: `make test-fe` (kontener `frontend`, Node ≥ 20). Pełny zestaw: `make test-all` (PHPUnit + Vitest).
